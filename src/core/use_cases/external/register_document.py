...
class RegisterDocumentValidator:
    ...
    def validate_data_logic(
        ...
        passengers_missing_ids: set[PassengerId] = self.base_validator.check_existence(
    ...
    def validate_business_logic(
        ...
        documents_retrieved_identity_keys: list[DocumentIdentityKey],
    ) -> None:
        exceptions: list[InvalidData] = []

        documents_missing_identity_keys: set[DocumentIdentityKey] = (
            self.base_validator.check_existence(
                documents_requested_identity_keys, documents_retrieved_identity_keys
            )
        )

        if (
            not set(documents_requested_identity_keys)
            == documents_missing_identity_keys
        ):
    ...
    def execute(self, document_request: DocumentRequest) -> DocumentResponse:
        with self.uow as uow:
            passengers_retrieved: list[Passenger] = (
                uow.passenger_repository.retrieve_passengers_by_id(
            ...
            self.register_document_validator.validate_data_logic(
                [document_request.passenger_id], passengers_retrieved_id
            )
            self.register_document_validator.validate_business_logic(
                [document_request.identity_key], documents_retrieved_identity_keys
            )
