...
class DummyBooking(BaseEntity):
    def __init__(self, reference: str, amount: Decimal, flight: DummyFlight, passenger: DummyPassenger, flights_list: list[DummyFlight] = None):
...
def test_to_dict_extracts_properties_and_ignores_internal_state(sample_flight, sample_uuid):
...
def test_to_dict_ignores_methods(sample_flight, sample_passenger):
    booking = DummyBooking(reference="B-1", amount=Decimal("100.00"), flight=sample_flight, passenger=sample_passenger)
...
def test_to_dict_handles_nested_entity_and_ignores_its_identity_key(sample_flight, sample_passenger, sample_uuid):
    booking = DummyBooking(reference="B-1", amount=Decimal("100.00"), flight=sample_flight, passenger=sample_passenger)
