...
def test_booking_router_no_exceptions(mocker: MockerFixture, client: TestClient, booking_request: BookingRequest, booking_response: BookingResponse) -> None:
    ...
    response = client.post("/api/bookings", json=booking_request.model_dump(mode="json"))

    assert response.status_code == 200
    assert response.json() == booking_response.model_dump(mode="json")

def test_booking_router_multiple_exceptions(mocker: MockerFixture, client: TestClient, booking_request: BookingRequest) -> None:
    mock_use_case = mocker.Mock()
    mock_use_case.execute.side_effect = MultipleExceptionsError([InexistentFlight(id=UUID("019f75c1-ab3f-7a29-a624-9da6c04e5a70")), 
