...
def test_document_router_no_exceptions(mocker: MockerFixture, client: TestClient, document_request: DocumentRequest, document_response: DocumentResponse) -> None:
    ...
    response = client.post("/api/documents", json=document_request.model_dump(mode="json"))

    assert response.status_code == 200
    assert response.json() == document_response.model_dump(mode="json")

def test_document_router_multiple_exceptions(mocker: MockerFixture, client: TestClient, document_request: DocumentRequest) -> None:
    mock_use_case = mocker.Mock()
    mock_use_case.execute.side_effect = MultipleExceptionsError([InexistentPassenger(id=UUID("019f75c1-ab3f-7a29-a624-9da6c04e5a70")),
