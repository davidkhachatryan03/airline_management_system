...
def test_retrieve_all_bookings(passenger_repository: PassengerRepository, passengers: list[Passenger]) -> None:
    passenger_repository.insert_passengers(passengers)

    all_inserted_passengers: list[Passenger] = passenger_repository.retrieve_passengers(limit=3)
