...
def test_insert_booking(booking_repository: BookingRepository, booking: Booking) -> None:
    ...
    assert type(last_inserted_booking.booking_datetime) == type(booking.booking_datetime)
...
def test_retrieve_all_bookings(booking_repository: BookingRepository, bookings: list[Booking]) -> None:
