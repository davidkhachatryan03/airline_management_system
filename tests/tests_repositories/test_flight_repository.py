...
def test_insert_flight(flight_repository: FlightRepository, flights: list[Flight]) -> None:
...
def test_retrieve_flights_by_id(
    ...
    flights_retrieved: list[Flight] = flight_repository.retrieve_flights_by_id(flight_ids)
...
def test_retrieve_flights_by_identity_key(
    ...
    flight_identity_keys: list[FlightIdentityKey] = [flight.identity_key for flight in flights]

    flights_retrieved: list[Flight] = flight_repository.retrieve_flights_by_identity_key(flight_identity_keys)

    assert set(flights) == set(flights_retrieved)


def test_retrieve_seats_available_per_flight(
        flight_repository: FlightRepository, flights: list[Flight], seats_available_per_flight_expected: dict[FlightId, int]
) -> None:
    flight_repository.insert_flights(flights)

    flight_ids: list[FlightId] = [flight.id for flight in flights]

    seats_available_per_flight: dict[FlightId, int] = flight_repository.retrieve_seats_available_per_flight(flight_ids)
